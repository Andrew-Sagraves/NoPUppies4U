...
void check_cron_jobs() {
    ...
    if (filesystem::exists(etccrontab)) process_file(etccrontab, checked_files);
...
string sanitize_token(string t) {
    ...
    while (!t.empty() && (t.back()=='|' || t.back()=='&' || t.back()=='>' || t.back()=='<' || t.back()==';' || t.back()==')')) t.pop_back();
...
vector<string> extract_paths(const string &cmd) {
    ...
    for (const string &tk : tokens) {
        ...
        if (s.empty()) continue;
        if (s.find('=') != string::npos && s.find('/') == string::npos) continue;
...
void process_file(const filesystem::path &p, set<string> &checked_files) {
    ...
    while (getline(f, line)) {
        ...
        if (eq != string::npos && sline.find(' ') == string::npos) continue;
        ...
        for (const string &pth : paths) {
            ...
            if (checked_files.count(candidate)) continue;
            ...
            if (filesystem::exists(candidate)) {
                ...
                cout << " (exists) ";
                if (writable) cout << "[WRITABLE]";
                else cout << "[not writable]";
                cout << endl;
            } else {
                cout << " (missing)" << endl;
            }
